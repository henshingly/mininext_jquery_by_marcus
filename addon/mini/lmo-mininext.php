  ...
  if ($file && $liga->loadFile(PATH_TO_LMO.'/'.$dirliga.$file) == TRUE) {
    ...
    if (is_null( $team_b = $liga->teamForNumber($b)) ) {
      ...
      foreach ($sortedGames as $game) {
		...
		$template->setVariable("saisongameTxt",$text['mini'][7]); // Es gibt ein zukünftiges Spiel
      ...
      if (!isset($partie) ) { // Keine weitere Partie gefunden, daher letzte Partie anzeigen (Saison beendet)
		...
		$template->setVariable("saisongameTxt",$text['mini'][2]);
      } else {
        $showLastGame = TRUE;
      }
    ...
    if (isset($partie) ) {
	  ...
	  if($last == 1) {
        $template->setVariable("lastPartie",$partie->hToreString()." : ".$partie->gToreString());
      }
     else {
       $template->setVariable("lastPartie", " - ");
     } 
