...
$mini_withArchiv = isset($_GET['mini_withArchiv']) ? ($_GET['mini_withArchiv'] == '1' ? 1 : 0) : (isset($mini_withArchiv) ? ($mini_withArchiv == '1' ? 1 : 0) : (isset($cfgarray['mini']['withArchiv']) ? $cfgarray['mini']['withArchiv'] : 1 ));

$mini_unGreedy = isset($_GET['mini_unGreedy']) ? ($_GET['mini_unGreedy'] == '1' ? 1 : 0) : (isset($mini_unGreedy) ? ($mini_unGreedy == '1' ? 1 : 0) : (isset($cfgarray['mini']['unGreedy']) ? $cfgarray['mini']['unGreedy'] : 1 ));

$mini_barWidth = isset($_GET['mini_barWidth']) && is_numeric($_GET['mini_barWidth']) ? $_GET['mini_barWidth']    : (isset($mini_barWidth) && is_numeric($mini_barWidth) ? $mini_barWidth : (isset($cfgarray['mini']['barWidth']) ? $cfgarray['mini']['barWidth'] : 120 ));
...
if (!file_exists($mini_cache_filename)
    ...
    if (basename($_SERVER['PHP_SELF'])=="lmo-mininext_jq.php") {?>
...
<body><?php
    ...
    if ($file && $liga -> loadFile(PATH_TO_LMO . '/' . $dirliga . $file) == true) {
        ...
        if (isset($partie) ) {
            ...
            if ($spAnzahl > $anzahl) {
                $linkmehr = '<tr id="tr_einausblenden">
          <td align="center">
            <div><a href="#" id="einausblenden" class="einblenden" title="alle Spiele aus dem Archiv einblenden">&nbsp;</a></div>
          </td>
        </tr>             ';
            }
            else {
...
/*
            ...
            $template ->setVariable("mehr",$linkmehr);
        }
